...
def main():
	...
	for r in relays:
		...
		try:
			...
			if r[0] == 'udp' and cfg.listen.startswith('127.0.0'):
				print("[!] In UDP, it's not recommended to bind to 127.0.0.1. If you see errors, try to bind to your LAN IP address instead.", 1)

		except:
			sys.exit('[!] error: Invalid relay specification, see help.')

	if not (cfg.cert and cfg.key):
		print("[!] Server cert/key not provided, SSL/TLS interception will not be available.", 1)

	if not (cfg.clientcert and cfg.clientkey):
		print("[!] Client cert/key not provided.", 1)
	...
	if cfg.proxy:
		...
		print("[!] Interception disabled! %s will run in monitoring mode only." % __prog_name__, 1)
	...
	while True:
		...
		try:
			...
			if packet.startswith('\x16\x03'): # SSL/TLS Handshake.
				...
				if not (cfg.cert and cfg.key):
					print("[!] SSL/TLS handshake detected, provide a server cert and key to enable interception.", 1)

				else:
					print('------------------ Wrapping sockets ------------------', 2)
...
def proxify(message, cfg, client_peer, server_peer, to_server=True):
	...
	def get_response():
		try:
			...
			print ("[!] error: can't connect to proxy!", 1)
			return message
	"""
	Modify traffic here
	...
	if cfg.script:
		...
		if new_message == None:
			print ('[!] Error: make sure handle_request and handle_response both return a message.', 1)
			new_message = message
